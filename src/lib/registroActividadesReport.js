...
export const generateBoardActivitiesHTML = (activities = [], { from = '', to = '', membreteUrl = '/fondo-oficios.jpg', signerName = 'M. A. Juan J. Reyes', signerRole = 'Coordinador', signerInstitution = 'Comisión de Acreditación Educación Continua, Colegio de Psicólogos de Guatemala', signerUrl = '', stampUrl = '' } = {}) => {
  const rows = sortActivitiesByDate(activities).map((activity, index) => `<tr style="background:${index % 2 ? '#f8fbfd' : '#fff'};">
    <td style="padding:5px 6px;border:1px solid #cbd5df;font-weight:700;line-height:1.35;">${escapeRegistroHTML(activity.actividad_nombre || 'Actividad sin nombre')}</td>
    <td style="padding:5px 6px;border:1px solid #cbd5df;line-height:1.35;">${escapeRegistroHTML(formatRegistroDate(activity.actividad_fecha))}</td>
    <td style="padding:5px 6px;border:1px solid #cbd5df;line-height:1.35;">${escapeRegistroHTML(activity.ponente_nombre || 'Pendiente')}</td>
    <td style="padding:5px 6px;border:1px solid #cbd5df;line-height:1.35;">${escapeRegistroHTML(activity.actividad_lugar || 'Pendiente')}</td>
    <td style="padding:5px 6px;border:1px solid #cbd5df;line-height:1.35;white-space:nowrap;">${escapeRegistroHTML(activity.actividad_hora || 'Pendiente')}</td>
  </tr>`).join('');
