...
import { activityDateKey, formatRegistroDate } from './registroActividadesReport.js';
...
// 'YYYY-MM' de una actividad del historial ('' si su fecha no es interpretable).
export const activityMonthKey = (fecha = '') => activityDateKey(fecha).slice(0, 7);
...
// actividades tiene cada uno para poder decidir sin abrirlos.
export const listActivityMonths = (activities = []) => {
  const counts = new Map();
  activities.forEach(activity => {
    const key = activityMonthKey(activity.actividad_fecha);
    if (!key) return;
...
export const activitiesInMonths = (activities = [], months = []) => {
  ...
  return activities
    .filter(activity => selected.has(activityMonthKey(activity.actividad_fecha)))
    .sort((a, b) => activityDateKey(a.actividad_fecha).localeCompare(activityDateKey(b.actividad_fecha))
